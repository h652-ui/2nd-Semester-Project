...
public class logincontroller implements Initializable{
    ...
    public void loginbuttonaction(ActionEvent actionEvent) throws IOException {
        if(!username.getText().isBlank() && !username.getText().isBlank()){
    ...
    public void validateInput(){
        ...
        String verifylogIn = "SELECT count(*) FROM login WHERE username='"+username.getText()+"'AND pasword='"+password.getText()+"'";
        String verifylogIn1 = "SELECT * FROM login WHERE username='"+username.getText()+"'AND pasword='"+password.getText()+"'";
                ...
                if(resultSet.getInt(1)==0) {
                    ...
                    try{
                        ...
                        while(resultquery.next()){
                            if(username.getText().equals(resultquery.getString("username"))&&password.getText().equals(resultquery.getString("pasword"))) {
                                Stage stage = (Stage) loginbutton.getScene().getWindow();
                                stage.close();
                                if (resultquery.getString("u_status").equals("resturant")) {
                                    userinformation.setWallet(Integer.parseInt(resultquery.getString("wallet")));
                                    userinformation.setU_name(resultquery.getString("firstname") + " " + resultquery.getString("lastname"));
                                    userinformation.setName(username.getText());
                                    ...
                                    rstage.show();
                                } else if (resultquery.getString("u_status").equals("student")) {
